...
enum { ZXC_CMP = 11 };
enum { ZXC_HOST_PTR = 19, ZXC_REQ_HANDLE = 20, ZXC_RES_HANDLE = 21 };
...
static int emit_bounds_check(uint8_t* out, size_t out_cap, size_t* out_len,
  ...
  int32_t imm19 = (int32_t)((payload_len + 4) / 4);
...
static int emit_primitive_prologue(uint8_t* out, size_t out_cap, size_t* out_len,
  ...
  if (!emit_mov_imm64(out, out_cap, out_len, 19, (int64_t)host_ptr)) return 0;
  if (prim_mask & PRIM_BIT_IN) {
    if (!emit_u32(out, out_cap, out_len, enc_mov_reg(20, 0))) return 0;
  }
  if (prim_mask & PRIM_BIT_OUT) {
    if (!emit_u32(out, out_cap, out_len, enc_mov_reg(21, 1))) return 0;
  }
  return 1;
}
    ...
    case 0x72: case 0x74: case 0x78: case 0x7A: case 0x7C:
      {
        ...
        sz = addr_sz + limit_sz + base_sz + 4 + ls_sz + 16;
      }
      break;
    ...
    case 0x91: /* FILL */
      {
        ...
        uint64_t limit = mem_size > 0 ? (mem_size - 1) : 0;
        size_t limit_sz = mov_imm64_size(limit);
        sz = base_sz + limit_sz + 10 * 4;
      }
      break;
    case ZOP_PRIM_IN: sz = 8 * 4; break;
    case ZOP_PRIM_OUT: sz = 9 * 4; break;
    case ZOP_PRIM_LOG: sz = 14 * 4; break;
    case ZOP_PRIM_ALLOC: sz = 2 * 4; break;
    case ZOP_PRIM_FREE: sz = 4 * 4; break;
    case ZOP_PRIM_CTL: sz = 13 * 4; break;
    default:
      sz = 4;
      break;
  }
  ...
  for (size_t off = 0; off < in_len; ) {
    ...
    switch (op) {
      ...
      case ZOP_JR: {
        ...
        if (cond == 14) {
          int64_t delta = (int64_t)target_out - (int64_t)(out_len + 4);
          if ((delta & 3) != 0) {
            res.err = ZXC_ERR_OPCODE;
            res.in_off = insn_off;
            res.out_len = out_len;
      ...
      case 0x7C: /* LD32S64 */ {
        ...
        if (!emit_bounds_trailer(out, out_cap, &out_len)) {
          res.err = ZXC_ERR_OUTBUF;
          res.in_off = insn_off;
          res.out_len = out_len;
          return res;
      ...
      case ZOP_RET:   enc = 0xD65F03C0u; break; /* ret */
      case ZOP_PRIM_IN: {
