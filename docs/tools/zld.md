...
Other primitives (e.g. `_log`, `_time`, `_crypto`) are rejected for now.
...
- `ZLD_ALLOW_PRIMS=in,out,log`

When a primitive is disabled, `CALL _in/_out/_log/_alloc/_free` fails with a hard error.
