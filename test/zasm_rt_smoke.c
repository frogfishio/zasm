...
static void test_instance_create_run_minimal_ret(void) {
  ...
  pol.target = ZASM_RT_TARGET_HOST;
...
static void test_target_mismatch_is_unsupported(void) {
  /* header(40) + dir(20) + CODE(4) */
  uint8_t buf[40 + 20 + 4] = {0};
  memcpy(buf, "ZASB", 4);
  put_u16_le(buf + 4, 2);
  ...
  pol.target = ZASM_RT_TARGET_X86_64;
#elif defined(__x86_64__) || defined(_M_X64)
  pol.target = ZASM_RT_TARGET_ARM64;
#else
  pol.target = ZASM_RT_TARGET_ARM64;
#endif
  ...
  zasm_rt_instance_t* inst = NULL;
  zasm_rt_err_t ce = zasm_rt_instance_create(e, m, &pol, NULL, &inst, &d);
  assert(ce == ZASM_RT_ERR_UNSUPPORTED);
  assert(inst == NULL);
...
int main(void) {
  ...
  test_target_mismatch_is_unsupported();
