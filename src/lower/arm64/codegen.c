...
static void emit_adrp_add(cg_blob_t *out,uint32_t *w,size_t *pcw,int rd,const char *sym){
  size_t off = (*pcw)*4;
  w[(*pcw)++] = 0x90000000u | (uint32_t)rd; /* ADRP rd, sym@PAGE */
  add_reloc(out,(uint32_t)off,0,sym);
  w[(*pcw)++] = 0x91000000u | ((uint32_t)rd<<5) | (uint32_t)rd; /* ADD rd, rd, sym@PAGEOFF */
  add_reloc(out,(uint32_t)(off+4),1,sym);
}
...
int cg_emit_arm64(const ir_prog_t *ir, cg_blob_t *out) {
  ...
  for (ir_entry_t *e = ir->head; e; e = e->next) {
    ...
    if (strcmp(m,"RET")==0) { w[pcw++] = enc_ret(); continue; }
      ...
      add_reloc(out,(uint32_t)(pcw*4),2, ops[0].sym);
      pcw++;
      if (to_spill) {
        size_t spill_bytes = to_spill*8;
        if (spill_bytes & 0xF) spill_bytes = (spill_bytes + 0xF) & ~0xFULL;
      ...
      if (nops==1) {
        ...
        if (imm19 < -0x40000 || imm19 > 0x3FFFF) {
          ...
          w[pcw] = enc_b_placeholder();
          add_reloc(out,(uint32_t)(pcw*4),2, ops[1].sym);
          pcw++;
        } else {
          uint8_t cond = cond_from_sym(ops[0].sym);
    ...
    if ((strcmp(m,"LD")==0 || strncmp(m,"LD",2)==0) && nops>=2) {
      ...
      if (ops[1].kind==IR_OP_SYM) {
        ...
        if (base < 0) { /* treat mem base as symbol */
          emit_adrp_add(out,w,&pcw,addr_reg,ops[1].mem_base);
        }
    ...
    if (strncmp(m,"ST",2)==0 && nops>=2) {
      ...
      if (base_is_sym) {
        addr_reg = 5;
        emit_adrp_add(out,w,&pcw,addr_reg,ops[0].mem_base);
      }
