...
Status: **STABLE for v2.0** (alpha). The goal is “boring ABI”: no breaking changes
within v2.0. New functionality should be added via new functions + feature bits.
...
### 0.0 Stability contract (normative)

Within ABI v2.0:
- Existing function names, argument meanings, return meanings, and error codes MUST NOT change.
- New optional functionality MUST be feature-gated (via `zi_abi_features()`), and MUST use new function names.
- Removing functions or changing pointer width semantics is a **breaking change** and would require an ABI major bump (e.g. v3.0).
...
#### 0.1.1 Portability note (non-normative; future integration)

The long-term portability goal is that “guest pointers” are *one concept across targets*.
Today v2.0 uses:
- 32-bit pointer arguments for core byte I/O (`zi_read/zi_write`) and many copy-out calls, and
- some 64-bit pointer arguments/returns for copy-out and Hopper.

This is stable for v2.0, but a future ABI major version may unify the pointer model
for native/JIT to match the WASM model (linear-memory offsets) to simplify embedders.
...
#### 0.2.1 Default stream handles are host-wired (normative)

Zing guest code MUST treat the reserved handles (`0/1/2`) as **default byte
streams**, not as “the console”.

The host/runner exclusively decides what these handles represent at runtime:
- terminal/console
- file
- pipe
- TCP socket
...
The guest MUST NOT assume:
- that handle `0` is interactive
- that handle `1` is a terminal
- any specific buffering, chunking, or device semantics

This is the intended portability point:
- **native** runners may implement this by wiring OS file descriptors (e.g. via
  `dup2` to map a file/socket/pipe onto fd `0`/`1`/`2`)
- **WASM/JIT** hosts implement `env.zi_read/zi_write` and route `(handle,ptr,len)`
  to the host’s attached stream objects
...
### 3.3 Example: host-wired stdio (non-normative)

The same guest program can be used as a file copier or as a network forwarder
purely by changing how the host wires the default handles:

- Shell redirection (native):
  - `./app < input.bin > output.bin`
- Runner wiring (native):
  - open a file/socket, then `dup2(fd, 0)` / `dup2(fd, 1)` before entering guest
- WASM host:
  - attach a file/socket object to “handle 0/1” inside the embedder, and implement
    `env.zi_read`/`env.zi_write` against those objects

Minimal native runner sketch (POSIX):

```c
int in_fd = open("in.bin", O_RDONLY);
int out_fd = open("out.bin", O_WRONLY|O_CREAT|O_TRUNC, 0644);
dup2(in_fd, 0);   /* stdin-like */
dup2(out_fd, 1);  /* stdout-like */
close(in_fd);
close(out_fd);
return zing_main(); /* enters guest */
```
...
## 6. Net capability (net/tcp) (alpha, native pack)

The native Zingcore pack ships an **async selector** capability for TCP:
...
These selectors are invoked via the async hub (see `stdlib/Zing/Async.zing` + `stdlib/Zing/Future.zing`).

### 6.1 Selector: `net.tcp.connect.v1`

Params (little-endian):

- `HBYTES host` (u32 len + bytes, no NUL)
- `H2 port`
- `H4 flags`

Flags (bitset, v1):
- bit0 `ALLOW_DNS` (if set, host may resolve non-IP hostnames)
- bit1 `PREFER_IPV6`
- bit2 `NODELAY`

Ok payload (little-endian):

- `H4 handle` (a stream handle that supports `zi_read/zi_write/zi_end`)
- `H4 hflags` (currently unused by the guest)
- `HBYTES meta` (currently empty)

### 6.2 Selector: `net.tcp.listen.v1`
...
- `H4 listener_fd` (POSIX listener fd; used as the accept handle)
- `H4 hflags`
- `HBYTES meta` where meta is:
  - `H2 bound_port`

Notes:
- v1 binds **loopback only** (`127.0.0.1` or `::1`).
- A future v2 may add explicit bind address / bind-any behavior.

### 6.3 Selector: `net.tcp.accept.v1`
...
- `H4 handle` (accepted connection as a stream handle; supports `zi_read/zi_write/zi_end`)
- `H4 hflags`
- `HBYTES meta` (currently empty)

### 6.4 Net policy hooks (native)

The native pack supports a host policy hook for outbound TCP connects:

```c
// Return 1 to allow, 0 to deny.
// Default implementation (weak) allows all.
int zi_net_policy_allow_connect(const uint8_t* host, uint32_t host_len, uint16_t port);
```

If denied, `net.tcp.connect.v1` fails with:
- code `t_net_denied`
- msg `policy`

### 6.5 Sandbox note (important)

Some development sandboxes disallow creating sockets entirely (e.g. `socket()` returns `EPERM`).
In such environments, `net.tcp.listen.v1` and `net.tcp.connect.v1` will fail with `t_net_denied` / `t_net_unreachable`.

This does not indicate a guest-side bug; it reflects host restrictions.
...
### 7.0 Host mapping (non-normative; recommended)

For native debug/integrator runtimes it is useful for the host to map the
guest-visible filesystem root (`/`) onto a host folder. This allows the guest
to operate on “all files” within the host’s chosen view without the guest
knowing the real host paths.

This repository’s native debug runtime supports:
- `ZI_FS_ROOT=/some/host/folder`

When set, guest paths like `/a/b.txt` are translated to:
`/some/host/folder/a/b.txt` (with `..` traversal rejected).

This is a host policy/translation mechanism (not a language feature).
...
fully locked in by the ABI core yet. The rule is: if a feature needs async, it
should be expressed as typed syscalls (not a framed selector protocol).

Integration note (non-normative): expect this section to expand (new `zi_async_*`
entrypoints + feature bits), but not break existing v2.0 functions.
...
Integration note (non-normative): future versions may add structured exec APIs
(argv/env arrays, working directory, exit status vs signals) as new entrypoints.
