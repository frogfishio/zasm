...
int test_async_net_echo(void) {
  uint32_t handle = 0;
  if (open_async_handle(900, &handle) != 0) return 200;
  uint8_t zax[256];
  const char kind[] = "net";
  const char name[] = "default";
  const char selector[] = "net.echo.v1";
  const char payload_data[] = "hello-net";
  uint32_t params_len = (uint32_t)sizeof(payload_data) - 1;
  ...
  write_zax_header(zax, 1, 1, 601, 9000, payload_zax);
  uint32_t zoff = 48;
  zax[zoff++] = 2;
  ...
  memcpy(zax + zoff, payload_data, params_len); zoff += params_len;
  ...
  while ((rn = req_read((int32_t)handle, resp, sizeof(resp))) > 0) {
    ...
    while ((size_t)rn - pos >= 48) {
      ...
      if (k == 2 && op == 101 && rid == 601) saw_ack = 1;
      if (k == 2 && op == 110 && fid == 9000) {
        if (plen < 4) return 202;
        uint32_t vlen = ctl_read_u32(pay);
        if (4u + vlen != plen) return 203;
        if (vlen != params_len) return 204;
        if (memcmp(pay + 4, payload_data, params_len) != 0) return 205;
        saw_ok = 1;
      }
      pos += 48u + plen;
    }
