...
static void maybe_init_time(void);
...
static int g_replay_time = 0;
static uint64_t g_replay_buf[256];
static size_t g_replay_len = 0;
static size_t g_replay_pos = 0;

static void maybe_init_time(void) {
  if (g_time_inited) return;
  const char* env = getenv("ZI_ASYNC_FAKE_TIME_MS");
  if (env && *env) {
    uint64_t v = (uint64_t)strtoull(env, NULL, 10);
    g_fake_time = 1;
    g_fake_now = v;
  }
  const char* replay = getenv("ZI_ASYNC_REPLAY_TIME_MS");
  if (replay && *replay) {
    FILE* f = fopen(replay, "r");
    if (f) {
      while (g_replay_len < sizeof(g_replay_buf)/sizeof(g_replay_buf[0])) {
        unsigned long long v = 0;
        if (fscanf(f, "%llu", &v) != 1) break;
        g_replay_buf[g_replay_len++] = (uint64_t)v;
      }
      fclose(f);
      if (g_replay_len > 0) g_replay_time = 1;
    }
  }
  g_time_inited = 1;
}
...
static uint64_t now_ms(void) {
  ...
  if (g_replay_time) {
    if (g_replay_pos < g_replay_len) {
      return g_replay_buf[g_replay_pos++];
    }
    return g_replay_len ? g_replay_buf[g_replay_len - 1] : 0;
  }
  if (g_fake_time) {
    g_fake_now += 10; /* deterministic stride */
    return g_fake_now;
  }
  struct timeval tv;
...
static int handle_join(async_handle_t* h, uint64_t req_id, const uint8_t* payload, uint32_t payload_len,
  ...
  if (timeout_ms > 0 && pending > 0) {
    ...
    uint64_t budget = (fuel == 0) ? 1u : fuel;
