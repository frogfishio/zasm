...
static int files_create_invoke(const zi_async_emit* emit, void* ctx,
  ...
  off += id_len;
  if (off + 4 > params_len) {
    return emit->future_fail(ctx, future_id, "t_async_bad_params", "data len");
  }
  uint32_t data_len = ctl_read_u32(params + off); off += 4;
  ...
  if (!zi_files_create(id, id_len, data, data_len)) {
    return emit->future_fail(ctx, future_id, "t_file_denied", "create");
  }
  return emit->future_ok(ctx, future_id, NULL, 0);
}

static int files_create_cancel(void* ctx, uint64_t future_id) {
  (void)ctx;
  (void)future_id;
  return 1;
}

static const zi_async_selector sel_files_create = {
  .cap_kind = "file",
  .cap_name = "view",
  .selector = "files.create.v1",
  .invoke = files_create_invoke,
  .cancel = files_create_cancel,
};

__attribute__((constructor))
static void files_create_autoreg(void) {
  zi_async_register(&sel_files_create);
}
