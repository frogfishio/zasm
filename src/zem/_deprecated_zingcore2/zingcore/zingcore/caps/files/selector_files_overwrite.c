...
static int files_overwrite_invoke(const zi_async_emit* emit, void* ctx,
  ...
  if (!zi_files_overwrite(id, id_len, data, data_len)) {
    return emit->future_fail(ctx, future_id, "t_file_not_found", "id");
  }
  return emit->future_ok(ctx, future_id, NULL, 0);
}

static int files_overwrite_cancel(void* ctx, uint64_t future_id) {
  (void)ctx;
  (void)future_id;
  return 1;
}

static const zi_async_selector sel_files_overwrite = {
  .cap_kind = "file",
  .cap_name = "fs",
  .selector = "files.overwrite.v1",
  .invoke = files_overwrite_invoke,
  .cancel = files_overwrite_cancel,
};

__attribute__((constructor))
static void files_overwrite_autoreg(void) {
  zi_async_register(&sel_files_overwrite);
}
