...
static int id_is_safe_component(const uint8_t* id, uint32_t id_len) {
...
static int id_has_zing_ext(const uint8_t* id, uint32_t id_len) {
  if (id_len < 5) return 0;
  return memcmp(id + (id_len - 5), ".zing", 5) == 0;
}
...
static int files_open_invoke(const zi_async_emit* emit, void* ctx,
  ...
  if (!id_is_safe_component(id, id_len) || !id_has_zing_ext(id, id_len)) {
    return emit->future_fail(ctx, future_id, "t_async_bad_params", "id");
  }
  const char* root = zi_files_root();
  if (!root) return emit->future_fail(ctx, future_id, "t_file_not_found", "root");
  char full[1024];
  if (snprintf(full, sizeof(full), "%s/%.*s", root, (int)id_len, (const char*)id) <= 0) {
    return emit->future_fail(ctx, future_id, "t_async_bad_params", "path");
  }
