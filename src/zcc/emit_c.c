...
static size_t label_pc_or_die(const labelvec_t* labels, const char* name, size_t ninstr) {
  ...
  if (pc > ninstr) pc = ninstr;
  return pc;
}

static void emit_data_segments(const datavec_t* data) {
  for (size_t i = 0; i < data->n; i++) {
    const data_seg_t* seg = &data->v[i];
    printf("static const uint8_t zprog_seg_%zu[] = {", i);
    for (size_t b = 0; b < seg->len; b++) {
      if (b % 16 == 0) printf("\n  ");
      printf("0x%02x", seg->bytes[b]);
      if (b + 1 != seg->len) printf(", ");
    }
    if (seg->len) printf("\n");
    printf("};\n\n");
  }
...
static void emit_mem_init(const datavec_t* data) {
  printf("static void zprog_state_init(struct zprog_state* state) {\n");
  printf("  memset(state, 0, sizeof(*state));\n");
  for (size_t i = 0; i < data->n; i++) {
    const data_seg_t* seg = &data->v[i];
    if (seg->len) {
      printf("  memcpy(state->mem + %u, zprog_seg_%zu, %zu);\n",
...
static void emit_symbol_enum(const gsymtab_t* g, namemap_t* map) {
  if (g->n == 0) return;
  printf("enum {\n");
  for (size_t i = 0; i < g->n; i++) {
...
static int emit_instruction(const record_t* r,
  ...
  if (strcmp(m, "LD") == 0) {
    ...
    if (dst->t == JOP_SYM && is_register(dst->s)) {
      ...
      if (src->t == JOP_NUM || (src->t == JOP_SYM && !is_register(src->s))) {
        ...
        printf("      if (!zprog_bounds(state.HL, 1)) return ZPROG_TRAP_OOB;\n");
        printf("      state.A = state.mem[state.HL];\n");
      } else {
    ...
    if (dst->t == JOP_MEM && dst->s && strcmp(dst->s, "HL") == 0) {
      if (src->t == JOP_SYM && strcmp(src->s, "A") == 0) {
        printf("      if (!zprog_bounds(state.HL, 1)) return ZPROG_TRAP_OOB;\n");
        printf("      state.mem[state.HL] = (uint8_t)(state.A & 0xff);\n");
        printf("      pc = %zu;\n      break;\n    }\n", next_pc);
        return 0;
      }
  ...
  if (strcmp(m, "CP") == 0) {
    ...
    const operand_t* rhs = &r->ops[1];
    if (rhs->t == JOP_NUM) {
      printf("      state.cmp = state.HL - %ld;\n", rhs->n);
    } else if (rhs->t == JOP_SYM && is_register(rhs->s)) {
      const char* src = reg_field(rhs->s);
      printf("      state.cmp = state.HL - %s;\n", src);
    } else {
      long v = 0;
      if (operand_value(rhs, g, &v) != 0) return 1;
      printf("      state.cmp = state.HL - %ld;\n", v);
  ...
  if (strcmp(m, "JR") == 0) {
    ...
    if (r->nops == 2) {
      ...
      size_t pc_target = label_pc_or_die(labels, target->s, next_pc);
      printf("      if (state.cmp %s 0) { pc = %zu; break; }\n", cmp, pc_target);
      printf("      pc = %zu;\n      break;\n    }\n", next_pc);
      return 0;
    }
  ...
  if (strcmp(m, "CALL") == 0) {
    ...
    if (strcmp(sym, "_in") == 0) {
      printf("      if (!in_fn) return ZPROG_TRAP_HOST_MISSING;\n");
      printf("      if (!zprog_bounds(state.HL, state.DE)) return ZPROG_TRAP_OOB;\n");
      printf("      int32_t n = in_fn(host_ctx, req_handle, state.mem, ZPROG_MEM_CAP, state.HL, state.DE);\n");
      printf("      if (n < 0) return ZPROG_TRAP_HOST_FAIL;\n");
      printf("      state.HL = n;\n");
      printf("      pc = %zu;\n      break;\n    }\n", next_pc);
      return 0;
    }
    if (strcmp(sym, "_out") == 0) {
      printf("      if (!out_fn) return ZPROG_TRAP_HOST_MISSING;\n");
      printf("      if (!zprog_bounds(state.HL, state.DE)) return ZPROG_TRAP_OOB;\n");
      printf("      if (out_fn(host_ctx, res_handle, state.mem, ZPROG_MEM_CAP, state.HL, state.DE) < 0) return ZPROG_TRAP_HOST_FAIL;\n");
      printf("      pc = %zu;\n      break;\n    }\n", next_pc);
      return 0;
    }
    if (strcmp(sym, "_log") == 0) {
      printf("      if (!log_fn) return ZPROG_TRAP_HOST_MISSING;\n");
      printf("      if (!zprog_bounds(state.HL, state.DE)) return ZPROG_TRAP_OOB;\n");
      printf("      if (!zprog_bounds(state.BC, state.IX)) return ZPROG_TRAP_OOB;\n");
      printf("      log_fn(host_ctx, state.mem, ZPROG_MEM_CAP, state.HL, state.DE, state.BC, state.IX);\n");
      printf("      pc = %zu;\n      break;\n    }\n", next_pc);
      return 0;
    }
    if (strcmp(sym, "_alloc") == 0) {
      printf("      if (!sys || !sys->alloc_fn) return ZPROG_TRAP_ALLOC;\n");
      printf("      int32_t ptr = sys->alloc_fn(sys->ctx, state.mem, ZPROG_MEM_CAP, state.HL);\n");
      printf("      if (ptr < 0) return ZPROG_TRAP_ALLOC;\n");
      printf("      state.HL = ptr;\n");
      printf("      pc = %zu;\n      break;\n    }\n", next_pc);
      return 0;
    }
    if (strcmp(sym, "_free") == 0) {
      printf("      if (!sys || !sys->free_fn) return ZPROG_TRAP_ALLOC;\n");
      printf("      sys->free_fn(sys->ctx, state.mem, ZPROG_MEM_CAP, state.HL);\n");
      printf("      pc = %zu;\n      break;\n    }\n", next_pc);
      return 0;
    }
    size_t pc_target = label_pc_or_die(labels, sym, next_pc);
    printf("      if (sp >= ZPROG_RET_STACK_CAP) return ZPROG_TRAP_CALL_DEPTH;\n");
    printf("      ret_stack[sp++] = %zu;\n", next_pc);
    printf("      pc = %zu;\n      break;\n    }\n", pc_target);
    return 0;
  }
  if (strcmp(m, "RET") == 0) {
    printf("      if (sp == 0) return 0;\n");
    printf("      pc = ret_stack[--sp];\n      break;\n    }\n");
    return 0;
  }
...
int emit_c_module(const recvec_t* recs, unsigned heap_slack) {
  datavec_t data;
  gsymtab_t g;
  ...
  printf("/* Autogenerated by zcc. Do not edit. */\n");
  printf("#include <stdint.h>\n#include <stddef.h>\n#include <string.h>\n#include \"zprog_rt.h\"\n\n");
  printf("#define ZPROG_MEM_CAP %u\n", mem_cap);
  printf("#define ZPROG_RET_STACK_CAP 256\n");
  printf("enum {\n  ZPROG_TRAP_OOB = 1,\n  ZPROG_TRAP_CALL_DEPTH = 2,\n  ZPROG_TRAP_HOST_MISSING = 3,\n  ZPROG_TRAP_HOST_FAIL = 4,\n  ZPROG_TRAP_ALLOC = 5\n};\n\n");
  printf("static inline int zprog_bounds(int32_t addr, int32_t len) {\n");
  printf("  if (addr < 0 || len < 0) return 0;\n");
  printf("  size_t end = (size_t)addr + (size_t)len;\n");
  printf("  return end <= ZPROG_MEM_CAP;\n}\n\n");
  printf("enum { ZPROG_HEAP_BASE = %u };\n\n", data.next_off);
  printf("static const uint32_t ZPROG_HEAP_BASE_CONST = ZPROG_HEAP_BASE;\n");
  printf("uint32_t zprog_heap_base_value(void) { return ZPROG_HEAP_BASE_CONST; }\n\n");
  printf("struct zprog_state {\n  int32_t HL;\n  int32_t DE;\n  int32_t A;\n  int32_t BC;\n  int32_t IX;\n  int32_t cmp;\n  uint8_t mem[ZPROG_MEM_CAP];\n};\n\n");
  emit_data_segments(&data);
  emit_mem_init(&data);
  namemap_t symmap;
  namemap_init(&symmap);
  emit_symbol_enum(&g, &symmap);

  printf("int lembeh_handle(int32_t req_handle,\n                  int32_t res_handle,\n                  zprog_in_fn in_fn,\n                  zprog_out_fn out_fn,\n                  zprog_log_fn log_fn,\n                  void* host_ctx,\n                  const struct zprog_sys* sys) {\n");
  printf("  (void)out_fn; /* may be unused */\n");
  printf("  (void)log_fn;\n");
  printf("  struct zprog_state state;\n  zprog_state_init(&state);\n");
  printf("  int32_t pc = 0;\n  int32_t ret_stack[ZPROG_RET_STACK_CAP];\n  int32_t sp = 0;\n  for (;;) {\n    switch (pc) {\n");

  for (size_t i = 0; i < instrs.n; i++) {
    size_t next_pc = (i + 1 < instrs.n) ? i + 1 : instrs.n;
    if (emit_instruction(instrs.v[i].rec, i, next_pc, &labels, &g, &symmap) != 0) {
