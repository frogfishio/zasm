...
The host MUST provide exactly these imports under the module name `"lembeh"`:
...
### 2.1 I/O Semantics (Normative)

- `req_read` MAY return fewer bytes than `dst_cap` (short read).
- `req_read` MUST return `0` on EOF and `-1` on error.
- If `dst_cap == 0`, `req_read` MUST return `0` and MUST NOT write any bytes.
- `res_write` MAY return fewer bytes than `src_len` (short write).
- `res_write` MUST return `-1` on error.
- If `src_len == 0`, `res_write` MUST return `0` and MUST NOT write any bytes.
- If the response stream is closed, `res_write` MUST return `-1` and MUST NOT perform a partial write.
- If a handle is invalid, `req_read/res_write/res_end` MUST return `-1` and MUST NOT crash.
- If `req_read` returns `-1`, it MUST NOT write any bytes into `dst_ptr`.
- `res_end` MUST be idempotent: calling it on a closed or already-ended handle MUST be a no-op (and MUST NOT crash).
- `log` is best-effort: the host MAY drop messages, but MUST NOT crash on invalid pointers or OOB access.
...
- All ABI calls that read/write guest memory MUST bounds-check pointers and lengths against `mem_cap`.
- On OOB pointer/length, the host MUST return `-1` (for calls with a return value) or perform a no-op (for `log` and `_free`).

**Determinism:** For identical call sequences, `_alloc` results MUST be deterministic.

### 3.1 Allocation Semantics (Normative)

- `_alloc(0)` MUST return `0` and MUST NOT change allocator state.
- `_alloc` MUST return pointers aligned to **4 bytes**.
- `_alloc` MUST return offsets within the guest byte space or `-1` on OOM.
- `_free(ptr)` MUST NOT crash for any input.
- `_free(0)` MUST be a no-op.
- If `ptr` does not match a prior `_alloc` return value, `_free` MUST be a no-op.
...
### 4.1 `_ctl` Framing Rules (Normative)
...
- If `payload_len` exceeds the available bytes, `_ctl` MUST return `#t_ctl_bad_frame` in a ZCL1 error envelope.
- On error envelopes, the ZCL1 response MUST include `ok=0` and the error fields (`trace`, `msg`, `cause`) in order.
...
### 4.2 `_ctl` Timeout Rules (Normative)

- `timeout_ms = 0` MUST be nonblocking: return immediately or fail with `#t_ctl_timeout`.
- For `timeout_ms > 0`, the host MAY block up to the timeout but MUST NOT exceed it.
...
- 0 = stdin (for `req_read`)
- 1 = stdout (for `res_write/res_end`)
- 2 = log stream (for `res_write/res_end`)
...
If multiple requests are in flight, the host MUST define a deterministic ordering
for `_ctl`, `req_read`, and `res_write` per handle. Implementations SHOULD serialize
...
- Hosts MUST define handle lifecycle: creation, validity, and invalidation semantics.
- Reused handles MUST NOT alias active handles.
...
- If `_ctl` receives an unsupported ZCL1 version, it MUST return `#t_ctl_bad_version`.
- There is no runtime ABI negotiation in v1; compliance is determined out-of-band
  by selecting the correct versioned guide.

## 12. Conformance Rules (Normative)

An implementation is compliant if all of the following hold:

- The core ABI surface matches exactly (names, signatures, and module name).
- `_alloc`/`_free` behavior follows Section 3.1.
- I/O semantics follow Section 2.1 (short read/write, EOF, closed stream).
- `_ctl` framing, error envelopes, and timeout rules follow Section 4.
- Reserved handle rules and lifecycle constraints follow Section 6.2.
...
**Envelope:** If the header is readable but invalid (`magic`, `v`, or `payload_len`), `_ctl` MUST return a ZCL1 error envelope.
